...
Label(root, text='Frequency = Hz', font=('Tahoma', 12), foreground='black').place(x=59, y=140)
Label(root, text='Execution Time = s', font=('Tahoma', 12), foreground='black').place(x=21, y=200)
...
def text():
    if Add_tcp.get() == 'tet-tcp://169.254.4.20':
        print('正确')
        return True
    else:
        print('错误')
...
Add_tcp_1 = Entry(root, textvariable=Add_tcp, validatecommand=text, font=('Tahoma', 14), width=45).place(x=158, y=40)
'''
添加图片路径
'''
...
def connect_eyetracker():
    ...
    eyetracker.set_gaze_output_frequency(Hz)  # 设置采样频率(Hz)
...
def gaze_data_callback(gaze_data):
    ...
    if count1 > 0 and point_x != 0 and point_y != 0 and (list_x_imgF[count1] - list_x_img[count1 - 1] > 10) and (
